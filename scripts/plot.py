...
@click.option('--plot_test_trans', is_flag=True, default=False, help='plot test.csv file')
# at
...
def plot(**kwargs):
    ...
    # plot test.csv file. 
        ...
        # ignore some columns
        ignore_columns = ['Unnamed: 0', 'time stamp', 'arch', 'path', 'loss', FLAGS.x]
        column_names = [column for column in df.columns if column not in ignore_columns]
    ...
    # when x = attack_eps, at_eps is fixed vise virsa
        ...
        # ignore some columns
        ignore_columns = ['Unnamed: 0', 'time stamp', 'path', 'loss', 'maxhinge_loss', 'at', 'at_norm', 'at_eps', 'attack_trg_loss', 'attack', 'attack_norm', 'attack_eps', 'cost']
        column_names = [column for column in df.columns if column not in ignore_columns]
